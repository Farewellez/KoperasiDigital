...
from model import to_csv_allProductDB
produk = to_csv_allProductDB()

import csv

produk = {}
with open('allProduk.csv', 'r') as file:
    reader = csv.DictReader(file)
    for row in reader:
        for key, value in row.items():
            if key not in produk:
                produk[key] = []
            produk[key].append(value)

from model import to_csv_allkeranjang
keranjang = to_csv_allkeranjang()
